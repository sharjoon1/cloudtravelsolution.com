...
<table width="600" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:8px;overflow:hidden;max-width:600px;width:100%;">
  <tr><td style="background:#0F6595;padding:24px 32px;">
    <h1 style="margin:0;color:#ffffff;font-size:20px;font-weight:600;">Cloud Travel Solution</h1>
  </td></tr>
  <tr><td style="padding:32px;">
    <h2 style="margin:0 0 16px;color:#0F6595;font-size:18px;">${title}</h2>
    ${body}
  </td></tr>
...
export async function sendLeadConfirmation(data: HeroLeadFormData): Promise<EmailResult> {
  ...
  const html = emailLayout(
    ...
    `<p style="color:#374151;font-size:15px;line-height:1.6;margin:16px 0 0;">
      In the meantime, feel free to reach us at <a href="tel:+919876543210" style="color:#0F6595;">+91 98765 43210</a> or reply to this email.
    </p>`
  );
...
export async function sendInquiryConfirmation(data: VisaInquiryFormData): Promise<EmailResult> {
  const html = emailLayout(
    ...
    `<p style="color:#374151;font-size:15px;line-height:1.6;margin:16px 0 0;">
      We'll reach out via your preferred method: <strong>${data.preferredContactMethod}</strong>. For urgent queries, call us at <a href="tel:+919876543210" style="color:#0F6595;">+91 98765 43210</a>.
    </p>`
  );
...
export async function sendContactConfirmation(data: ContactFormData): Promise<EmailResult> {
  const html = emailLayout(
    ...
    <p style="color:#374151;font-size:15px;line-height:1.6;margin:0;">
      For urgent matters, call us at <a href="tel:+919876543210" style="color:#0F6595;">+91 98765 43210</a>.
    </p>`
  );
...
export async function sendNewsletterWelcome(email: string): Promise<EmailResult> {
  const html = emailLayout(
    ...
    <p style="color:#374151;font-size:15px;line-height:1.6;margin:0;">
      Planning a trip? <a href="${process.env.NEXT_PUBLIC_SITE_URL || "http://localhost:3000"}" style="color:#0F6595;font-weight:600;">Visit our website</a> to get started.
    </p>`
  );
