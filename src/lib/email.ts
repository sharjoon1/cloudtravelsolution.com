...
export async function sendLeadNotification(data: HeroLeadFormData): Promise<EmailResult> {
  ...
  const html = emailLayout(
    ...
    `<p style="margin:16px 0 0;"><a href="${process.env.NEXT_PUBLIC_SITE_URL || "http://localhost:3000"}/admin/collections/leads" style="display:inline-block;padding:10px 20px;background:#0cfcbc;color:#0c6cbc;text-decoration:none;border-radius:6px;font-size:14px;font-weight:600;">View in Admin</a></p>`
  );
...
export async function sendLeadConfirmation(data: HeroLeadFormData): Promise<EmailResult> {
  ...
  const html = emailLayout(
    ...
    `<p style="color:#374151;font-size:15px;line-height:1.6;margin:0 0 16px;">
      We received your travel inquiry for <strong>${dest}</strong>. Our team will contact you within <strong style="color:#0c6cbc;">2 hours</strong> to discuss your trip.
    </p>
    ...
    `<p style="color:#374151;font-size:15px;line-height:1.6;margin:16px 0 0;">
      In the meantime, feel free to reach us at <a href="tel:+919876543210" style="color:#0c6cbc;">+91 98765 43210</a> or reply to this email.
    </p>`
  );
...
export async function sendInquiryNotification(data: VisaInquiryFormData): Promise<EmailResult> {
  const html = emailLayout(
    ...
    `<p style="margin:16px 0 0;"><a href="${process.env.NEXT_PUBLIC_SITE_URL || "http://localhost:3000"}/admin/collections/inquiries" style="display:inline-block;padding:10px 20px;background:#0cfcbc;color:#0c6cbc;text-decoration:none;border-radius:6px;font-size:14px;font-weight:600;">View in Admin</a></p>`
  );
...
export async function sendInquiryConfirmation(data: VisaInquiryFormData): Promise<EmailResult> {
  const html = emailLayout(
    ...
    `<p style="color:#374151;font-size:15px;line-height:1.6;margin:16px 0 0;">
      We'll reach out via your preferred method: <strong>${data.preferredContactMethod}</strong>. For urgent queries, call us at <a href="tel:+919876543210" style="color:#0c6cbc;">+91 98765 43210</a>.
    </p>`
  );
...
export async function sendCallbackNotification(data: CallbackFormData): Promise<EmailResult> {
  const html = emailLayout(
    ...
    `<p style="color:#0c6cbc;font-size:14px;font-weight:600;margin:16px 0 0;">Please call them as soon as possible.</p>`
  );
...
export async function sendContactNotification(data: ContactFormData): Promise<EmailResult> {
  const html = emailLayout(
    ...
    `<div style="margin:16px 0;padding:16px;background:#f9fafb;border-left:4px solid #0c6cbc;border-radius:4px;">
...
export async function sendContactConfirmation(data: ContactFormData): Promise<EmailResult> {
  const html = emailLayout(
    ...
    `<p style="color:#374151;font-size:15px;line-height:1.6;margin:0 0 16px;">
      We received your message and will respond within <strong style="color:#0c6cbc;">24 hours</strong>.
    </p>
    ...
    <p style="color:#374151;font-size:15px;line-height:1.6;margin:0;">
      For urgent matters, call us at <a href="tel:+919876543210" style="color:#0c6cbc;">+91 98765 43210</a>.
    </p>`
  );
...
export async function sendNewsletterWelcome(email: string): Promise<EmailResult> {
  const html = emailLayout(
    ...
    <p style="color:#374151;font-size:15px;line-height:1.6;margin:0;">
      Planning a trip? <a href="${process.env.NEXT_PUBLIC_SITE_URL || "http://localhost:3000"}" style="color:#0c6cbc;font-weight:600;">Visit our website</a> to get started.
    </p>`
  );
