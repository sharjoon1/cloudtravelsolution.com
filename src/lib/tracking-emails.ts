...
export async function sendServiceRequestCreatedEmail(params: {
  ...
  const html = emailLayout(
    ...
    dataTable(
      dataRow("Tracking Code", `<strong style="color:#0066FF;font-size:16px;">${trackingCode}</strong>`) +
    ...
    `<p style="color:#374151;font-size:15px;line-height:1.6;margin:16px 0 0;">
      ...
      <a href="${siteUrl}/track" style="color:#0066FF;font-weight:600;">${siteUrl}/track</a>
    </p>
    <p style="margin:16px 0 0;">
      <a href="${siteUrl}/partner/dashboard" style="display:inline-block;padding:10px 20px;background:#0066FF;color:#ffffff;text-decoration:none;border-radius:6px;font-size:14px;font-weight:600;">View in Partner Portal</a>
    </p>`
  );
...
export async function sendStatusUpdateEmail(params: {
  ...
  const html = emailLayout(
    ...
    `<p style="margin:16px 0 0;">
      <a href="${siteUrl}/partner/dashboard" style="display:inline-block;padding:10px 20px;background:#0066FF;color:#ffffff;text-decoration:none;border-radius:6px;font-size:14px;font-weight:600;">View Details</a>
    </p>`
  );
